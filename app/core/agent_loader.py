...
# In-memory agent cache to eliminate Firestore latency on every call
_agent_cache: Dict[str, dict] = {}

def load_agent(agent_id: str) -> Optional[Dict[str, Any]]:
    """
    Load a single agent by ID from Firestore (with 30s in-memory cache).
    ...
    try:
        ...
        if agent_id_str in _agent_cache:
            cache_entry = _agent_cache[agent_id_str]
            if now - cache_entry['timestamp'] < 30.0:
                print(f"[AgentLoader] ⚡ Returning cached agent '{agent_id_str}' (TTL: {30.0 - (now - cache_entry['timestamp']):.1f}s remaining)")
...
def get_default_agent() -> Optional[Dict[str, Any]]:
    """
    Get the first active agent as fallback (with 30s cache).
    ...
    try:
        ...
        if agent_id_str in _agent_cache:
            cache_entry = _agent_cache[agent_id_str]
            if now - cache_entry['timestamp'] < 30.0:
                print(f"[AgentLoader] ⚡ Returning cached default agent (TTL: {30.0 - (now - cache_entry['timestamp']):.1f}s remaining)")
                return cache_entry['data']
        ...
        for doc in query:
            ...
            _agent_cache[agent_id_str] = {'timestamp': now, 'data': agent}
            return agent
