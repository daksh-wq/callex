...
from app.audio.classifier import SoundEventClassifier  # Import classifier
from app.audio.vad_silero import SileroVADFilter  # Production VAD
from app.audio.semantic import SemanticFilter  # Filler word filter
...
from app.managers.scripts import get_script_manager
...
# We go two directories up from __file__ (app/main.py) to get to root, then /recordings
...
try:
    DASHBOARD_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "enterprise", "frontend", "dist")
    if os.path.exists(DASHBOARD_DIR):
        ...
        print(f"[DASHBOARD] Served at http://0.0.0.0:8085/dashboard/ and http://0.0.0.0:8085/")
        print(f"[DASHBOARD] Files: {', '.join(os.listdir(DASHBOARD_DIR)[:5])}...")
    else:
        print(f"[DASHBOARD] Warning: Enterprise dashboard directory not found at {DASHBOARD_DIR}")
