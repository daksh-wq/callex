...
async def freeswitch_break(uuid: str):
    """Immediately stop all audio playback on a FreeSWITCH channel.
    
    uuid_break interrupts whatever is currently playing on the channel,
    which is the ONLY reliable way to stop buffered audio mid-playback.
    """
    if not uuid:
        return
    try:
        ...
        if b"+OK" not in auth_response:
            ...
            await writer.wait_closed()
            return
        # uuid_break stops ALL queued audio on the channel instantly
        cmd = f"api uuid_break {uuid} all\n\n"
        writer.write(cmd.encode())
        await writer.drain()
        response = await reader.readuntil(b"\n\n")
        print(f"[ESL] ⚡ uuid_break sent for {uuid}: {response.decode().strip()}")
        writer.close()
        await writer.wait_closed()
    except Exception as e:
        print(f"[ESL] uuid_break failed for {uuid}: {__safe_log(e)}")
...
async def _handle_call(ws: WebSocket, route_agent_id: str = None):
    ...
    async def cancel_current():
        nonlocal current_task, bot_speaking, bot_audio_expected_end, barge_in_active
        # IMMEDIATELY block all outgoing audio
        barge_in_active = True
        bot_speaking = False
        bot_audio_expected_end = time.time()
        
        # Send stop signals to FreeSWITCH FIRST (before waiting for task)
        if ws_alive:
            try:
                await ws.send_json({"event": "clear"})
                await ws.send_json({"type": "STOP_BROADCAST", "stop_broadcast": True})
                print("[SYSTEM] ✅ CLEAR + STOP_BROADCAST sent")
            except Exception as e:
                print(f"[SYSTEM] STOP_BROADCAST send failed: {__safe_log(e)}")
        
        # CRITICAL: Use FreeSWITCH ESL uuid_break to kill audio playback on the phone
        if call_uuid:
            try:
                await freeswitch_break(call_uuid)
            except Exception as e:
                print(f"[SYSTEM] uuid_break failed: {__safe_log(e)}")
        ...
        async with task_lock:
            if current_task and not current_task.done():
                ...
                current_task.cancel()
                try:
                    await asyncio.wait_for(current_task, timeout=0.3)
    ...
    async def send_audio_safe(audio_chunk: bytes) -> bool:
        ...
        if barge_in_active:
            return False
        try:
            chunk_duration = len(audio_chunk) / 32000.0
            now = time.time()
            if bot_audio_expected_end < now:
                bot_audio_expected_end = now
            
            # ── REAL-TIME AUDIO PACING ──
            # Instead of blasting all audio at network speed (0.5s for 5s of audio),
            # pace it so FreeSWITCH only has ~300ms of buffered audio at any time.
            # This way, when barge-in stops sending, audio cuts off within 300ms.
            MAX_LEAD_SECONDS = 0.3  # Maximum seconds of audio ahead of real-time
            lead_time = bot_audio_expected_end - now
            if lead_time > MAX_LEAD_SECONDS:
                sleep_time = lead_time - MAX_LEAD_SECONDS
                # Sleep in small increments so we can check barge_in_active frequently
                sleep_start = time.time()
                while (time.time() - sleep_start) < sleep_time:
                    if barge_in_active or not ws_alive:
                        return False
                    await asyncio.sleep(min(0.05, sleep_time - (time.time() - sleep_start)))
    ...
    if client:
        ...
        async def _flush_number_accumulator():
            ...
            try:
                ...
                async for audio_chunk in tts_stream_generate(client, reply_text, voice_id=agent_config['voice'], agent_voice_speed=agent_config.get('voiceSpeed'), agent_language=agent_config.get('language')):
                    if barge_in_active:
                        print("[TTS-ACCUM] 🛑 Barge-in active, aborting TTS stream")
                        break
                    if not ws_alive:
                        break
        ...
        async def _process_sst_model_2_transcripts():
            ...
            while ws_alive:
                try:
                    ...
                    # Reset barge_in_active AFTER cancel completes so TTS loop sees it as True
                    barge_in_active = False
        ...
        if os.path.exists(cache_path):
            ...
            print(f"[CACHE] Generating opener on the fly for agent {agent_config['id']} and SAVING to cache")
            ...
            try:
                # Open tmp file for writing while we stream
                with open(tmp_path, "wb") as cache_file:
                    async for chunk in tts_stream_generate(client, opener_text, voice_id=agent_config['voice'], agent_voice_speed=agent_config.get('voiceSpeed'), agent_language=agent_config.get('language')):
                        if barge_in_active:
                            print("[TTS] 🛑 Barge-in active, aborting Opener TTS stream")
                            break
                ...
                # ONLY if we made it through without error, atomically move to cache path
                if not barge_in_active:
                    os.rename(tmp_path, cache_path)
            except Exception as e:
                print(f"[CACHE] Error writing opener cache: {e}")
        ...
        try:
            while ws_alive:
                ...
                if "bytes" in msg:
                    ...
                    # BARGE-IN FIX: Always feed audio to STT even during bot speech
                    # so customer interruptions get transcribed immediately
                    ...
                    if audio_db > INTERRUPTION_THRESHOLD_DB:
                        if not speaking:
                            ...
                            # IMMEDIATELY flush FreeSWITCH buffer — don't wait for cancel_current
                            if ws_alive:
                                try:
                                    await ws.send_json({"event": "clear"})
                                    await ws.send_json({"type": "STOP_BROADCAST", "stop_broadcast": True})
                                    print("[BARGE-IN] ⚡ Instant CLEAR+STOP sent")
                                except Exception:
                                    pass
                            
                            # CRITICAL: uuid_break kills FreeSWITCH audio playback IMMEDIATELY
                            if call_uuid:
                                try:
                                    await freeswitch_break(call_uuid)
                                except Exception:
